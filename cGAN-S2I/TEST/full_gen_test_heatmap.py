...
from GENERATOR.gen_def_RESNET_CBAM import CBAMResNetGenerator
...
def test_generator_with_heatmap(model_path, test_root, mask_subfolder, heatmap_subfolder, image_subfolder, roi_yaml_path,
    ...
    dataset = MaskImageDataset(
        ...
        heatmap_subfolder=heatmap_subfolder,
        image_subfolder=image_subfolder
    )
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
    roi_list = load_rois(roi_yaml_path)

    generator = CBAMResNetGenerator(input_nc=2, output_nc=3).to(device)
    generator.load_state_dict(torch.load(model_path, map_location=device))
    generator.eval()

    metrics_global = {
        'LPIPS': [], 'Laplacian Var Diff': [], 'Gradient L1 Loss': [],
        'PSNR': [], 'SSIM': [], 'IoU': [], 'Dice': [], 'Chamfer': []
    }
    metrics_roi = {
        'IoU': [], 'Dice': [], 'Chamfer': [], 'Grad L1': [],
        'LPIPS': [], 'PSNR': [], 'SSIM': [], 'Laplacian Var Diff': []
    }
    ...
    with torch.no_grad():
        for i, (mask, heatmap, real) in enumerate(tqdm(loader)):
            mask, heatmap, real = mask.to(device), heatmap.to(device), real.to(device)
            input_tensor = torch.cat([mask, heatmap], dim=1)
            ...
            if fake_crops and real_crops:
                for fake_crop, real_crop in zip(fake_crops, real_crops):
                    ...
                    if h >= 16 and w >= 16:
                        lpips_val = lpips_loss(fake_crop.unsqueeze(0), real_crop.unsqueeze(0)).item()
                    ...
                    metrics_roi['PSNR'].append(psnr_metric(r_img, f_img, data_range=255))
                    metrics_roi['SSIM'].append(ssim_metric(r_img, f_img, channel_axis=-1, data_range=255))
...
if __name__ == "__main__":
    ...
    test_generator_with_heatmap(
        model_path=r"C:\Users\johan\Downloads\GENERATORS_TRAINED\DARK\RESNET_CBAM_HEATBIN\generator_lr_0_0005.pth",
        test_root="DATA",
        mask_subfolder="MASKS_DARK_200/TEST_SET",
        heatmap_subfolder="HEATMAP_DARK_200/TEST_SET",
        image_subfolder="DARK_IMG_200/TEST_SET",
        roi_yaml_path="CONFIG/DARK_TEST_SET_annotated_rois.yaml",
        batch_size=4,
        save_imgs=True,
        save_dir="EVAL_RESULTS/DARK/HEATMAP_RESNETCBAM_lr_5e-04_DARK_50"
    )