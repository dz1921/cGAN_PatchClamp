...
class CBAM(nn.Module):
    def __init__(self, channels, reduction=16, kernel_size=7):
        ...
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # global pooling over H×W
    ...
    def forward(self, x):
        ...
        ca = self.channel_attention(avg + max_)
        x = x * ca
...
# Double convolution block with CBAM enhancement
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=0.3):
        super().__init__()

        # Two convolutional layers with normalisation and dropout
    ...
    def forward(self, x):
        x = self.conv(x)
        return self.cbam(x)  # Attention refinement
...
class UNetPPGenerator(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_filters=64, dropout=0.3):
        ...
        # Final 1×1 convolution + Tanh activation for image output
        self.final = nn.Conv2d(f, out_channels, kernel_size=1)
        self.tanh = nn.Tanh()

    def forward(self, x):
        # Encoder forward (backbone)
        x00 = self.conv00(x)
